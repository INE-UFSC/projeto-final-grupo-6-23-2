...
class Button:
    def __init__(
        ...
        posicao: tuple,
        click_method,
        img_static: str = "button-static.png",
        img_hover: str = "button-hover.png",
        text: str = None,
    ) -> None:
        """Essa classe serve de base para criação de botões, tornando possível
        instanciar botões customizáveis."""
        ...
        self.__image = pygame.image.load(
            f"versao_final/styles/assets/telas/buttons/{self.__image_static}"
        ).convert_alpha()
        ...
        self.__sound_click = pygame.mixer.Sound(
            "versao_final/styles/assets/sound_effects/button-click.wav"
        )

        if not self.__text is None:
            self.__text_surface = self.__font.render(
                self.__text, True, self.__text_color
            )
            self.__text_rect = self.__text_surface.get_rect()
            self.__text_rect.x = (
                self.__rect.x + self.rect.width / 2 - self.__text_rect.width / 2
            )
            self.__text_rect.y = (
                self.__rect.y + self.rect.height / 2 - self.__text_rect.height / 2
            )
    ...
    def update(self, event):
        ...
        if self.__rect.collidepoint(mouse_pos):
            if clicked:
                ...
                self.__image = pygame.image.load(
                    f"versao_final/styles/assets/telas/buttons/{self.__image_hover}"
                ).convert_alpha()
