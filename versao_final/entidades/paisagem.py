...
class Layer(Sprite):
    def __init__(self, file: str, alpha=False):
        super().__init__()
        if alpha:
            self.image = pygame.image.load(os.path.join(os.path.dirname(__file__), "../styles/assets/parallax-background/"+file)).convert_alpha()
        else:
            self.image = pygame.image.load(os.path.join(os.path.dirname(__file__), "../styles/assets/parallax-background/"+file)).convert()
